...
import course from './spMapping/courseSP';
...
app.get('/searchGrade', (req, res) => {
    pool.getConnection((err, connection) => {
        connection.query(grade.search, [ req.query.couName ], (err, result) => {
            res.send(result[0]);
            connection.release();
        });
    });
...
// API /courseList: Get all course
app.get('/courseList', (req, res) => {
    pool.getConnection((err, connection) => {
        connection.query(course.select, (err, result) => {
            res.send(result[0]);
            connection.release();
        });
    });
